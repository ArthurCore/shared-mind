...
| `PYTHONPATH=src python3 -m unittest discover -s tests -v` | 536 tests, 0 failures/errors; 1 pre-existing optional MCP SDK v1 skip. |
...
does not add web streaming, review promotion, pruning, model extraction, automatic
summary injection, or an Agent-specific state table. Actual Claude hook payloads must
